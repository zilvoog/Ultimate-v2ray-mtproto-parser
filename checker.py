...
async def check_http(semaphore, proto, config):
    """Проверка конфига через SOCKS5 прокси"""
    async with semaphore:
        try:
            # Поддержка доменов и IP
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
                # generate_204 надежнее в GHA, чем google.com
...
async def main():
    ...
    # СОРТИРОВКА: сначала по протоколу, потом по конфигу
